...
//    var itemArray = [Item]()
    ...
    var todoItems : Results<Item>? // Auto Obtaining container. ! kullanınca yani implicitly unwrapped optional yapınca aşağıda todoItems?.count ?? 1 böyle nil gelmesi ihtimaline karşı(loadItems() commentli kalırsa) kontrol koymak konusunda sana kızmıyor(todoItems.count buna kızmıyor) Çünkü zaten unwrap ettin nil gelmez diye güven verdin diyor. Nil gelirse patlar yani. Ama ? koyunca explicitly unwrapped optional yapınca todoItems.count böyle bırakamazsın, nil gelebilir diyor ve seni todoItems?.count ?? 1 şeklinde yazmaya zorluyor.
    
    let realm = try! Realm()
    
    var selectedCategory : Category? {  //An explicitly unwrapped optional category object
...
//        return todoItems?.count == 0 || todoItems?.count == nil ? 1 : todoItems!.count
...
// CoreData creating new item and save it code
...
extension TodoListViewController: UISearchBarDelegate{
    ...
    func searchBarSearchButtonClicked(_ searchBar: UISearchBar) {
        ...
        todoItems = todoItems?.filter("title CONTAINS[cd] %@", searchBar.text!).sorted(byKeyPath: "dateCreated", ascending: true) //Realm mantığıyla bu kadar kısa, aşağıdaki kodların yerine tek satır kod ile hallettik
        
        tableView.reloadData()
        
        

//       CoreData,NSManaged için
//        let request : NSFetchRequest<Item> = Item.fetchRequest()
//
